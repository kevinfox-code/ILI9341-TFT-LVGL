...
> **Historical note (v2.1.0):** this plan was written and executed while the
> `tft_drivers` library lived at the repository root and the reference
> example lived under `examples/LVGL_Lesson/01_Display/`. All 9 phases below
> are DONE against that layout. A later restructure (see `RELEASE_NOTES.md`
> v2.1.0) moved this library one level down to `App/drivers/` and promoted
> the example to the repo root — every path in §3 "Target Layout" below is
> now one directory deeper than written (e.g. `include/drv/` here is
> `App/drivers/include/drv/` today). No code or acceptance criteria changed.
...
> **Status: DONE.** Written, with skeleton `TouchGFXGeneratedHAL`-derived flush
> class and `TouchController::sampleTouch` skeleton per the spec's three bullets.
