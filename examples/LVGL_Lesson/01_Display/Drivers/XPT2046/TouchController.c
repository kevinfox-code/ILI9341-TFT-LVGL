...
void touch_calibrate(void) {
    /* MUST be called from LVGL task or before vTaskStartScheduler(). */
#ifdef FREERTOS
    ...
    for(int i = 0; i < 4; i++) {
        ...
        lv_timer_handler();
        lv_refr_now(NULL);
        DELAY_MS(100);  /* Small delay to ensure display update completes */
        ...
        DELAY_MS(100);  /* Small delay between points */
    }
...
/* PENIRQ EXTI handler: notify LVGL task (no SPI or LVGL calls here). */
